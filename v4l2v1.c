...
static int read_frame_dmabuf_mp(int deviceFd, enum v4l2_buf_type type,
                                struct buffer_mp *bufs, __s32 *fds,
                                unsigned int n_buffers) {
  DEBUG("-- read_frame_dmabuf_mp(%d, %s, %p, %d)\n", deviceFd,
  ...
  if (type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
    fprintf(stderr, "DMABUF input not supported");
    return -1;
  }
...
static int start_capturing_dmabuf_mp(int deviceFd, enum v4l2_buf_type type,
                                   struct buffer_mp *bufs, size_t n_bufs,
                                   void *source) {
  DEBUG("-- start_capturing_dmabuf_mp(%d, %s, %p, %zu, %p)\n", deviceFd,
        type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE ? "cap" : "out", bufs,
        n_bufs, source);
  unsigned int i;

  for (i = 0; i < n_bufs; ++i) {
    printf("start_capturing_dmabuf_mp buffer: #%d, (%p) size: %u\n", i,
    ...
    if (type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
      fprintf(stderr, "does not support input buffers\n");
      return -1;
    }
...
static int init_dmabuf_mp(int deviceFd, enum v4l2_buf_type type,
                          struct buffer_mp **bufs_out, __s32 *fds,
                          unsigned int n_bufs) {
  ...
  if (req.count * FMT_NUM_PLANES != n_bufs) {
...
static int init_device_out(struct Decoder *decoder) {
  ...
  init_dmabuf_mp(decoder->deviceFd, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE,
                 &decoder->buffers_mp_out, decoder->fds,
                 decoder->n_buffers_out);
...
static void free_buffers_dmabuf(struct Decoder *decoder,
                              enum v4l2_buf_type type) {
  struct v4l2_requestbuffers req = {0};
...
static int handle_event(struct Decoder *decoder) {
  ...
  do {
    ...
    case V4L2_EVENT_SOURCE_CHANGE:
      ...
      free_buffers_dmabuf(decoder, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);

      init_dmabuf_mp(decoder->deviceFd, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE,
                   &decoder->buffers_mp, decoder->fds, decoder->n_buffers);

      start_capturing_dmabuf_mp(
          decoder->deviceFd, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE,
          decoder->buffers_mp, decoder->n_buffers, decoder->output);
      break;
    case V4L2_EVENT_EOS:
...
int init_decoder(struct Decoder *decoder) {
  ...
  init_dmabuf_mp(deviceFd, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE,
               &decoder->buffers_mp, decoder->fds, decoder->n_buffers);
